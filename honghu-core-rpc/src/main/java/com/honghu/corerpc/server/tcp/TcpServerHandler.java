...
public class TcpServerHandler implements Handler<NetSocket> {
    ...
    public void handle(NetSocket socket) {
        TcpHandlerBufferWrapper bufferWrapper = new TcpHandlerBufferWrapper(buffer ->{
           //处理请求码
            ...
            try {
                Buffer encode = ProtocolMessageEncoder.encoder(responseProtocolMessage);
                socket.write(encode);
            } catch (IOException e) {
                throw new RuntimeException("协议消息编码错误");
            }
        });
        socket.handler(bufferWrapper);
    }
}
